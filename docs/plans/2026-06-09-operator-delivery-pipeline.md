...
**Migration numbering:** `075_creatives.sql` already exists on this branch. W1 schema = **076**, W2 schema = **077**. *(Post-merge note 2026-06-10: renumbered to **079**/**080** at ship time — main had independently consumed 076–078 for the Bunny Stream work. Applied to prod as 079_clients_brokerage / 080_delivery_pipeline. Inline SQL blocks below retain the original 076/077 headers as historical record.)* Every migration task ends with: *apply via Supabase (service-role REST/MCP) before dependent tasks run.* Repo has **no** generic `updated_at` trigger convention (verified: `clients.ts` sets `updated_at` explicitly in `updateClient`; 064 just defaults `now()`) — new lib code sets `updated_at` explicitly on UPDATE, no triggers.
...
## Task 3: Migration 079 (`clients.brokerage`) + brand-kit Brand.phone + client editor fields

**Files:**
- Create: `supabase/migrations/079_clients_brokerage.sql`
...
- [ ] Write the migration `supabase/migrations/079_clients_brokerage.sql` (full SQL):
  ```sql
...
- [ ] Update `lib/operator-studio/brand-kit.ts`:
  ...
  export function brandKitFromClient(c: ClientRow, ctx: { brokerage?: string | null }): BrandKitVars {
    return {
      ...
      // Precedence (migration 079): client's own brokerage → property brokerage.
...
- [ ] Commit:
  ```bash
  git add supabase/migrations/079_clients_brokerage.sql lib/types/operator-studio.ts lib/operator-studio/brand-kit.ts lib/operator-studio/__tests__/brand-kit.test.ts src/components/studio/ClientPicker.tsx src/pages/dashboard/studio/ClientEdit.tsx
...
## Task 5: Migration 080 — `delivery_runs`, `scene_variants`, `ml_events`

**Files:**
- Create: `supabase/migrations/080_delivery_pipeline.sql`
- Modify: `lib/types/operator-studio.ts` (append row types)

**Steps:**

- [ ] Create `supabase/migrations/080_delivery_pipeline.sql` (full SQL — enums as CHECK constraints, matching the 062/064 repo convention; no updated_at triggers — lib code sets it, matching `updateClient`):
  ```sql
...
- [ ] Commit:
  ```bash
  git add supabase/migrations/080_delivery_pipeline.sql lib/types/operator-studio.ts
  git commit -m "feat(delivery): migration 080 — delivery_runs + scene_variants + ml_events (service-role RLS)
