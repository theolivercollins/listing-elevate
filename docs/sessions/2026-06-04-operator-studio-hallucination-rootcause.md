...
4. **Re-render-on-hallucination loop** — `resubmitScene` extracted (shared by the resubmit endpoint + cron); on a judge hard-reject the cron makes ONE corrective re-render (default `MAX_QC_RERENDERS=2` = original + 1 retry) then surfaces `needs_review`. The loop is a safety NET, not a self-healer: a v1.1 re-render reuses the same push-in SKU + source frame, so the real hallucination prevention is the pre-render analyzer gate (#6) + director coercion (#5); the judge guarantees nothing bad ships silently. (97ebbf7)
