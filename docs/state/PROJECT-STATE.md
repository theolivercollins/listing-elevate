...
Last updated: **2026-06-26 (Operator Autopilot / auto-run — branch `feat/operator-autopilot-autorun`, NOT YET MERGED): confidence-gated autonomous resolver for the video operator dashboard. `delivery_runs.auto_run=true` enables a cron sweep (`api/cron/auto-run-sweep`) + inline kicks to drive each pipeline gate (checkpoint_a, details, voiceover, music, checkpoint_b) to `delivered`, setting `paused_reason` on low-confidence gates. Per-run `resolving_at` lease (migration 091) prevents double-spend. LLM picks (Haiku) + resumed renders write `cost_events`. New operator actions: `set_auto_run`, `resume_autopilot`. UI: intake checkbox, command-center Autopilot badge, enable-from-off control, decision log, pause/resume. Migrations 090 (`delivery_runs.auto_run` / `paused_reason` / `auto_paused_at` + ml_events CHECK extension), 091 (`resolving_at` lease), 092 (`assembly_h_job` / `assembly_v_job` tracking) — **APPLIED to shared prod DB 2026-06-28**. Migration 095 (`is_test boolean` on `properties` + `cost_events`, set via `isNonProdEnv()`, filtered from live operator queue/dashboard and all cost reconciliation/margin views) — **APPLIED 2026-06-28**. Env changes 2026-06-28: `LE_ALLOW_NONPROD_WRITES=true` on Vercel Preview (dev + staging branches); `CRON_SECRET` set on Production. Autopilot verified live on dev preview. PR #132 open into main; next = merge + verify prod cron. Spec: `docs/specs/2026-06-26-operator-autopilot-autorun-design.md`.** (Previous: 2026-06-19 (TypeScript baseline CI gate — branch `claude/romantic-gates-271307`, merged to main): `pnpm typecheck` / `pnpm typecheck:baseline` now gate CI on every PR + push to main/staging/dev via `.github/workflows/typecheck.yml`. Gate is count-aware: 101 pre-existing error occurrences across 55 file+code+message signatures grandfathered in `.typecheck-baseline.json`; any new signature or increased occurrence count fails the build. Addresses the historical "no build-time typecheck" incident class (see project memory). FOLLOW-UP: burn down the 55 baseline entries as errors are fixed. No migrations, no env changes.** (Previous: 2026-06-18 (studio hardening batch — branch `claude/trusting-visvesvaraya-0ba659`, awaiting PR review + merge-to-main): (1) `api/admin/studio/template-availability.ts` (new, requireAdmin) + `isTemplateConfigured()` in `lib/assembly/template-resolver.ts` — studio form only offers Creatomate combos that have a configured env var; prevents silent code-gen fallback for unconfigured durations/packages. (2) `splitAddress` bug in `lib/assembly/template-modifications.ts` fixed — comma+unit addresses (`456 Oak Ave, Apt B, Tampa, FL`) now emit correct `cityState='Tampa, FL'` instead of the bare last token (`FL`); 41 tests green. (3) Post-approval compute decoupled from the 300s approve-POST: `api/pipeline/continue/[runId].ts` (new, maxDuration=300) receives a fire-and-forget from approve; `runScripting` failures now write `setRunError`; director Anthropic call now records a `cost_event`; `reapStuckGeneratingDeliveryRuns` in `lib/pipeline/stuck-reaper.ts` recovers runs stuck `generating` >15 min (zero scenes OR all scenes needs_review + no clip). (4) `passingThreshold(totalScenes) = ceil(totalScenes*0.8)` replaces hardcoded `passed < 6` in `api/cron/poll-scenes.ts`; `reapStuckGeneratingDeliveryRuns` wired into the cron alongside sibling reapers. No migrations, no env changes.) (Previous: 2026-06-14 durability + missing-vids — branch harden/durability, PR #119 open awaiting main-merge approval): (1) Checkpoint A scene-card `<video>` lacked `autoPlay` — browser loaded metadata only, painted blank grey frames; fixed by adding `autoPlay`+`preload="metadata"` in `src/components/studio/CheckpointA.tsx`. (2) Judge transient-retry: `judgePair` wrapped in `withJudgeRetry` in `lib/delivery/judge.ts` — 429/5xx/timeout no longer fail a delivery judge pass; operator winner picks preserved. (3) Atlas-402 surfacing: `AtlasInsufficientBalanceError` classified PERMANENT and guarded at all 4 call sites (`submitScene`, `resubmitScene`, `submitVariantsForProperty`, `regenerateVariant`) — insufficient-balance 402 now fails loudly instead of silently degrading to lower-quality fallback. No migrations.) For current right-now state and the complete shipping log, read HANDOFF.md first. Earlier header updates: 2026-06-12 (MU merge to main: Market Update workflow complete E2E; migrations table 082 + 084 + 085; feat/le-video docs-sync gate). Earlier body entry: 2026-04-28 (lab cost-tracking fix + ledger-driven system update merged to main, commit `cd242fc`).
Last updated: **2026-06-28 (Security Hardening Phase 0 — migrations 093/094 APPLIED TO PROD; DB is now RLS-protected on all 57 tables; client access = owner/admin-scoped via JWT; server access = service-role bypasses RLS unchanged; 5 views revoked + security_invoker; Phase-0 code fixes deploying via this promotion; remaining backlog F7–F32 in follow-on. See HANDOFF.md for full detail.)** (Previous: 2026-06-19 (TypeScript baseline CI gate — branch `claude/romantic-gates-271307`, merged to main): `pnpm typecheck` / `pnpm typecheck:baseline` now gate CI on every PR + push to main/staging/dev via `.github/workflows/typecheck.yml`. Gate is count-aware: 101 pre-existing error occurrences across 55 file+code+message signatures grandfathered in `.typecheck-baseline.json`; any new signature or increased occurrence count fails the build. Addresses the historical "no build-time typecheck" incident class (see project memory). FOLLOW-UP: burn down the 55 baseline entries as errors are fixed. No migrations, no env changes.** (Previous: 2026-06-18 (studio hardening batch — branch `claude/trusting-visvesvaraya-0ba659`, awaiting PR review + merge-to-main): (1) `api/admin/studio/template-availability.ts` (new, requireAdmin) + `isTemplateConfigured()` in `lib/assembly/template-resolver.ts` — studio form only offers Creatomate combos that have a configured env var; prevents silent code-gen fallback for unconfigured durations/packages. (2) `splitAddress` bug in `lib/assembly/template-modifications.ts` fixed — comma+unit addresses (`456 Oak Ave, Apt B, Tampa, FL`) now emit correct `cityState='Tampa, FL'` instead of the bare last token (`FL`); 41 tests green. (3) Post-approval compute decoupled from the 300s approve-POST: `api/pipeline/continue/[runId].ts` (new, maxDuration=300) receives a fire-and-forget from approve; `runScripting` failures now write `setRunError`; director Anthropic call now records a `cost_event`; `reapStuckGeneratingDeliveryRuns` in `lib/pipeline/stuck-reaper.ts` recovers runs stuck `generating` >15 min (zero scenes OR all scenes needs_review + no clip). (4) `passingThreshold(totalScenes) = ceil(totalScenes*0.8)` replaces hardcoded `passed < 6` in `api/cron/poll-scenes.ts`; `reapStuckGeneratingDeliveryRuns` wired into the cron alongside sibling reapers. No migrations, no env changes.) (Previous: 2026-06-14 durability + missing-vids — branch harden/durability, PR #119 open awaiting main-merge approval): (1) Checkpoint A scene-card `<video>` lacked `autoPlay` — browser loaded metadata only, painted blank grey frames; fixed by adding `autoPlay`+`preload="metadata"` in `src/components/studio/CheckpointA.tsx`. (2) Judge transient-retry: `judgePair` wrapped in `withJudgeRetry` in `lib/delivery/judge.ts` — 429/5xx/timeout no longer fail a delivery judge pass; operator winner picks preserved. (3) Atlas-402 surfacing: `AtlasInsufficientBalanceError` classified PERMANENT and guarded at all 4 call sites (`submitScene`, `resubmitScene`, `submitVariantsForProperty`, `regenerateVariant`) — insufficient-balance 402 now fails loudly instead of silently degrading to lower-quality fallback. No migrations.) For current right-now state and the complete shipping log, read HANDOFF.md first. Earlier header updates: 2026-06-12 (MU merge to main: Market Update workflow complete E2E; migrations table 082 + 084 + 085; feat/le-video docs-sync gate). Earlier body entry: 2026-04-28 (lab cost-tracking fix + ledger-driven system update merged to main, commit `cd242fc`).
Last updated: **2026-06-28 (Security overhaul complete — Phase 0 (migrations 093/094 APPLIED; RLS on all 57 tables, 5 views locked, route auth guards F2/F3/F4/F5/F11/F12) + backlog batch (migrations 095/096 APPLIED; search_path pinned on 14 functions F16, anon EXECUTE revoked on 2 RPCs F15, SSRF allowlist F8, blog filter injection F25, PII logging removed F23). Advisor scan = 0 critical / 0 high. See HANDOFF.md for full detail.)** (Previous: 2026-06-19 (TypeScript baseline CI gate — branch `claude/romantic-gates-271307`, merged to main): `pnpm typecheck` / `pnpm typecheck:baseline` now gate CI on every PR + push to main/staging/dev via `.github/workflows/typecheck.yml`. Gate is count-aware: 101 pre-existing error occurrences across 55 file+code+message signatures grandfathered in `.typecheck-baseline.json`; any new signature or increased occurrence count fails the build. Addresses the historical "no build-time typecheck" incident class (see project memory). FOLLOW-UP: burn down the 55 baseline entries as errors are fixed. No migrations, no env changes.** (Previous: 2026-06-18 (studio hardening batch — branch `claude/trusting-visvesvaraya-0ba659`, awaiting PR review + merge-to-main): (1) `api/admin/studio/template-availability.ts` (new, requireAdmin) + `isTemplateConfigured()` in `lib/assembly/template-resolver.ts` — studio form only offers Creatomate combos that have a configured env var; prevents silent code-gen fallback for unconfigured durations/packages. (2) `splitAddress` bug in `lib/assembly/template-modifications.ts` fixed — comma+unit addresses (`456 Oak Ave, Apt B, Tampa, FL`) now emit correct `cityState='Tampa, FL'` instead of the bare last token (`FL`); 41 tests green. (3) Post-approval compute decoupled from the 300s approve-POST: `api/pipeline/continue/[runId].ts` (new, maxDuration=300) receives a fire-and-forget from approve; `runScripting` failures now write `setRunError`; director Anthropic call now records a `cost_event`; `reapStuckGeneratingDeliveryRuns` in `lib/pipeline/stuck-reaper.ts` recovers runs stuck `generating` >15 min (zero scenes OR all scenes needs_review + no clip). (4) `passingThreshold(totalScenes) = ceil(totalScenes*0.8)` replaces hardcoded `passed < 6` in `api/cron/poll-scenes.ts`; `reapStuckGeneratingDeliveryRuns` wired into the cron alongside sibling reapers. No migrations, no env changes.) (Previous: 2026-06-14 durability + missing-vids — branch harden/durability, PR #119 open awaiting main-merge approval): (1) Checkpoint A scene-card `<video>` lacked `autoPlay` — browser loaded metadata only, painted blank grey frames; fixed by adding `autoPlay`+`preload="metadata"` in `src/components/studio/CheckpointA.tsx`. (2) Judge transient-retry: `judgePair` wrapped in `withJudgeRetry` in `lib/delivery/judge.ts` — 429/5xx/timeout no longer fail a delivery judge pass; operator winner picks preserved. (3) Atlas-402 surfacing: `AtlasInsufficientBalanceError` classified PERMANENT and guarded at all 4 call sites (`submitScene`, `resubmitScene`, `submitVariantsForProperty`, `regenerateVariant`) — insufficient-balance 402 now fails loudly instead of silently degrading to lower-quality fallback. No migrations.) For current right-now state and the complete shipping log, read HANDOFF.md first. Earlier header updates: 2026-06-12 (MU merge to main: Market Update workflow complete E2E; migrations table 082 + 084 + 085; feat/le-video docs-sync gate). Earlier body entry: 2026-04-28 (lab cost-tracking fix + ledger-driven system update merged to main, commit `cd242fc`).
...
## DB access model (updated 2026-06-28)

- **Client-side (anon/authenticated JWT):** RLS enforced on all 57 tables (migration 093, applied 2026-06-28). Policies: `properties` = owner-or-admin SELECT; 5 finance tables = admin-only; remaining tables locked (no public policies). `public.is_admin()` helper checks `user_profiles.role = 'admin'`. 5 views require caller-rights evaluation (`security_invoker=true`, migration 094). Any new table or view must define explicit RLS policies before going live.
- **Server-side (service-role):** service-role key bypasses RLS entirely — unchanged. All server API routes use the service-role client; auth/ownership is enforced in route middleware (`requireAdmin`, `requireOwnerOrAdmin`), not at the DB layer.
- **Before 2026-06-28:** RLS was disabled on all tables; anon key held full DML across the entire DB. This is now closed.

## Known bugs / gotchas

- - **OPEN (2026-06-27): Bunny assembly failure — no-Referer 403.** Bunny library 679131 blocks requests with no `Referer` header (`BlockNoneReferrer=true` is the account default). Creatomate's render server fetches clip CDN URLs without a `Referer` → HTTP 403 → assembly fails; `delivery_runs.horizontal_video_url` / `vertical_video_url` stay null. Mitigation: PropertyCommandCenter now surfaces individual scene clips when video URLs are null (branch worktree-fix-continue-hop-race, cc0c086). Fix pending: flip `BlockNoneReferrer=false` via Bunny account-level API key, or add a clip-proxy endpoint. Requires Oliver to supply a Bunny account API key.
- **Delivery-run generation stalls (fixed in worktree-fix-continue-hop-race, pending merge).** The approve→continue HTTP hop (fire-and-forget POST to `api/pipeline/continue/[runId]`) is silently dropped when the Vercel lambda suspends between Checkpoint-A approval and `continuePipelineAfterPhotoSelection` kicking off. Delivery runs stuck at `generating` with zero scenes then hung indefinitely. Fix: `reapStuckGeneratingDeliveryRuns` in `lib/pipeline/stuck-reaper.ts` (commit 6ca1bc0) auto-re-fires `continuePipelineAfterPhotoSelection` for zero-scene stalls after 5 min, with a 45-min give-up window; studio `retry` action hardened with atomic CAS claim (b5211ed). No schema changes.
- **compute_units untracked-spend bug (P0, 2026-06-19)** — `unit_type='compute_units'` is emitted by the MLS scrapers (`lib/mls/scrape-realtor.ts`, `lib/mls/scrape-redfin.ts`, `lib/compass/scrape-listing.ts`) but is missing from the `cost_events_unit_type_check` constraint, so all Apify/Browserbase scraping spend has been silently dropped in prod since launch. Caught by `lib/__tests__/cost-constraint-drift.test.ts` (carved into `KNOWN_PENDING_DRIFT`). Fix: add `compute_units` to the constraint in a future migration (migrations 090–095 are now applied — Autopilot schema, RLS hardening, is_test marker — next available is 096). See `docs/plans/2026-06-19-observability-system-plan.md`.
- **OPEN (2026-06-27): Bunny assembly failure — no-Referer 403.** Bunny library 679131 blocks requests with no `Referer` header (`BlockNoneReferrer=true` is the account default). Creatomate's render server fetches clip CDN URLs without a `Referer` → HTTP 403 → assembly fails; `delivery_runs.horizontal_video_url` / `vertical_video_url` stay null. Mitigation: PropertyCommandCenter now surfaces individual scene clips when video URLs are null (branch worktree-fix-continue-hop-race, cc0c086). Fix pending: flip `BlockNoneReferrer=false` via Bunny account-level API key, or add a clip-proxy endpoint. Requires Oliver to supply a Bunny account API key.
- **Delivery-run generation stalls (fixed in worktree-fix-continue-hop-race, pending merge).** The approve→continue HTTP hop (fire-and-forget POST to `api/pipeline/continue/[runId]`) is silently dropped when the Vercel lambda suspends between Checkpoint-A approval and `continuePipelineAfterPhotoSelection` kicking off. Delivery runs stuck at `generating` with zero scenes then hung indefinitely. Fix: `reapStuckGeneratingDeliveryRuns` in `lib/pipeline/stuck-reaper.ts` (commit 6ca1bc0) auto-re-fires `continuePipelineAfterPhotoSelection` for zero-scene stalls after 5 min, with a 45-min give-up window; studio `retry` action hardened with atomic CAS claim (b5211ed). No schema changes.
...
| 090 | `autopilot_autorun` | `delivery_runs.auto_run boolean DEFAULT false`, `paused_reason text`, `auto_paused_at timestamptz`; `ml_events` CHECK extended with `auto_run_decision` event type. **Applied to shared Supabase 2026-06-28.** Branch `feat/operator-autopilot-autorun`. |
| 091 | `autopilot_lease` | `delivery_runs.resolving_at timestamptz` — per-run lease preventing double-spend from overlapping `auto-run-sweep` cron ticks. **Applied to shared Supabase 2026-06-28.** Branch `feat/operator-autopilot-autorun`. |
| 092 | `autopilot_assembly_tracking` | `delivery_runs.assembly_h_job text`, `delivery_runs.assembly_v_job text` — tracks in-flight Creatomate assembly job IDs for resume on the autopilot path. **Applied to shared Supabase 2026-06-28.** Branch `feat/operator-autopilot-autorun`. |
| 093 | `security_rls_lockdown_f1` | **APPLIED TO PROD 2026-06-28 (vrhmaeywqsohlztoouxu). VERIFIED: anon REST returns 401 on all locked tables.** RLS enabled on all 57 previously-public tables. Anon + authenticated DML grants revoked. `public.is_admin()` helper added (checks `user_profiles.role = 'admin'`). `properties` table: owner-or-admin SELECT policy. 5 finance tables (`token_purchases`, `expenses`, `revenue_entries`, `subscriptions`, `cost_events`): admin-only SELECT/INSERT/UPDATE/DELETE policies. Server-side service-role is unaffected (bypasses RLS). Closes security audit finding F1 (whole DB was world-readable/writable via the anon key). Branch `worktree-security-hardening`. |
| 094 | `security_view_lockdown_f6` | **APPLIED TO PROD 2026-06-28 (vrhmaeywqsohlztoouxu). NUMBERING COLLISION: branch `feat/provider-logging-cost-price-accuracy` also has a "094" migration — Supabase keys on timestamp so applied state is correct, but filenames will drift when that branch lands; rename its 094 before merging.** Revokes anon/authenticated SELECT grants on 5 public views (`lab_prompt_override_readiness`, `prompt_lab_iterations_complete`, `v_judge_calibration_status`, `v_knowledge_map_cells`, `v_rated_pool`) that previously bypassed RLS via owner-rights evaluation. Sets `security_invoker = true` on all five so future queries evaluate the caller's RLS context instead of the view owner's. Closes security audit finding F6. Branch `worktree-security-hardening`. |
| 095 | `is_test_marker` | `is_test boolean DEFAULT false` on `properties` and `cost_events`; set at insert when `isNonProdEnv()` (lib/env.ts) is true. Filtered from live operator queue/dashboard views (prod-only) and always filtered from cost reconciliation/margin views. Lets dev/preview runs write to the shared DB without polluting live metrics; bulk-clean: `DELETE FROM properties WHERE is_test`. **Applied to shared Supabase 2026-06-28.** Branch `feat/operator-autopilot-autorun`. |
| 095 | `security_function_search_path_f16` | **APPLIED TO PROD 2026-06-28 (vrhmaeywqsohlztoouxu).** Pins `search_path` on 14 app functions to prevent search-path injection (finding F16). Closes the function-level search-path attack surface left open after the RLS lockdown. Branch `worktree-security-backlog`. |
| 096 | `security_revoke_internal_rpc_exec_f15` | **APPLIED TO PROD 2026-06-28 (vrhmaeywqsohlztoouxu).** Revokes EXECUTE on `claim_v21_outcomes` + `increment_creative_view` from anon + authenticated roles; service_role EXECUTE retained. These RPCs were callable by any client without authentication. Closes security audit finding F15. Branch `worktree-security-backlog`. |
| 097 | `drive_telegram_intake` | New tables: `drive_watch_state` (one row per watched channel — channel_id, resource_id, expiry, last_page_token), `drive_intake_properties` (Drive folder → intake lifecycle: folder_id, folder_name, status CHECK, telegram_message_id, property_id FK, redfin_data JSONB, created_at/updated_at), `drive_intake_photos` (Drive file → photo row mapping: intake_id FK, drive_file_id, file_name, photo_id FK, created_at). Supports Drive → Telegram HITL → pipeline intake loop. **Applied to shared Supabase 2026-06-26** (originally as 090; renamed to 097 to avoid collision with autopilot migrations above). Feature gated behind `DRIVE_INTAKE_ENABLED` env var (currently unset/false). Branch `feat/drive-telegram-intake-ship`. File: `supabase/migrations/097_drive_telegram_intake.sql`. |
| 098–102 | *(not yet documented in this table — see HANDOFF.md shipping log for details)* | 098 RLS perf-advisor cleanup, 099 `user_profiles.avatar_url`, 100 role-escalation fix, 101 studio drafts, 102 welcome emails. |
| 103 | `walkthrough_mode` | Adds opt-in `pipeline_mode='walkthrough'`: single continuous multi-reference walkthrough video generated from a property's photos via Atlas SKU `seedance-reference-walkthrough` (Bytedance Seedance 2.0 reference-to-video), bypassing the per-scene v1/v1.1 pipeline entirely. Widens `properties_pipeline_mode_check` to `('v1', 'v1.1', 'walkthrough')`; adds `walkthrough_status` (null\|processing\|complete\|failed), `walkthrough_video_url`, `walkthrough_job_id` (Atlas prediction id), `walkthrough_error`, `walkthrough_updated_at` on `properties`. Fully additive — v1/v1.1 behavior untouched. **Applied to shared Supabase 2026-07-01.** Branch `feat/walkthrough-mode` (PR #156, not yet merged). Rollback: `103_walkthrough_mode_rollback.sql` (restores the v1/v1.1-only CHECK; requires migrating any `walkthrough` rows back to `v1` first) + drops the `walkthrough_*` columns. |
