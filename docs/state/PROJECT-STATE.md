...
| 090 | `drive_telegram_intake` | New tables: `drive_watch_state` (one row per watched channel — channel_id, resource_id, expiry, last_page_token), `drive_intake_properties` (Drive folder → intake lifecycle: folder_id, folder_name, status CHECK, telegram_message_id, property_id FK, redfin_data JSONB, created_at/updated_at), `drive_intake_photos` (Drive file → photo row mapping: intake_id FK, drive_file_id, file_name, photo_id FK, created_at). Supports Drive → Telegram HITL → pipeline intake loop. **Applied to shared Supabase 2026-06-26.** Feature gated behind `DRIVE_INTAKE_ENABLED` env var (currently unset/false). Branch `worktree-feat+drive-telegram-intake`. File: `supabase/migrations/090_drive_telegram_intake.sql`. |
