...
3. **Phase B — model head-to-head** — scope narrowed by Window D Round 1 audit (2026-04-21, `docs/audits/router-coverage-2026-04-21.md`). Existing 170 rated iterations yield **zero** (room × movement × SKU) buckets passing the winner rule (>=3 iter, >=80% 4★+). Round 2 (2026-04-21) seeded the targeted grid via `scripts/seed-router-grid.ts` — listing `1746b7de-ac3f-45e0-baae-db5b168c4cb9` with 5 scenes and 22 planned iterations across kitchen/living_room/master_bedroom/exterior_front × push_in + aerial × drone_push_in. **12 submitted, 10 blocked on Atlas 402** — wallet exhausted mid-grid. Oliver to top up Atlas + re-run `seed-router-grid.ts --write` (idempotent; only retries the 10 failed) to complete the grid. Rating-prep at `docs/briefs/2026-04-21-rating-session-prep.md`. After rating, `scripts/build-router-table.ts --write` emits the real router-table. `lib/providers/router.ts` stays on intuition-based routing until that lands.
