...
- **compute_units untracked-spend bug (P0, 2026-06-19)** — `unit_type='compute_units'` is emitted by Apify/Browserbase scraping (`lib/mls/scrape-realtor.ts`, `scrape-redfin.ts`, `lib/compass/scrape-listing.ts`) but is absent from the `cost_events_unit_type_check` DB constraint. Every scraping cost_event silently fails a 23514 CHECK and is swallowed by `.catch()`. Apify/Browserbase spend has never been recorded in prod. Fix: migration 090 adds `compute_units` to the constraint. Tracked as KNOWN_PENDING_DRIFT in `lib/__tests__/cost-constraint-drift.test.ts`. See `docs/plans/2026-06-19-observability-system-plan.md`.
