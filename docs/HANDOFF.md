...
- **Homepage Ally — `worktree-feat+homepage-ally` branch (2026-05-15).** Public concierge chat lifted from in-app blog Ally; feature-flagged off by default (`VITE_HOMEPAGE_ALLY_ENABLED`). Backend at `api/marketing/ally-chat.ts` (Sonnet 4.6 + prompt caching, server-canonical thread to block prompt-injection), widget at `src/components/marketing/MarketingAllyChat.tsx`, mounted in `src/v2/pages/Landing.tsx`. Daily cleanup cron at `0 4 * * *` (registered in `vercel.json`). All 16 plan tasks shipped + per-task code reviews + final full-branch review (2 Critical + 1 Important caught and fixed: `vercel.json` buildCommand wired to `pnpm build` so the prebuild generates `pricing.json`/`faq.json`; `knowledge.md` reconciled to match the visible page (24h turnaround, 20–60 photos); server now uses Supabase-stored thread as canonical context). Migration `057_marketing_ally.sql` **APPLIED to prod Supabase 2026-05-15** (RPC + tables verified live). `IP_HASH_SALT` env var set in Vercel dev + preview (production deferred). 24/24 Vitest tests pass. Spec: `docs/specs/2026-05-15-homepage-ally-design.md`. Plan: `docs/plans/2026-05-15-homepage-ally-plan.md`. Next: (1) merge to `dev` → preview deploy → run `scripts/marketing/test-ally-chat.ts` against the preview URL (5 scenarios incl. "no Helgemo leakage" regression guard); (2) merge `dev` → `staging` → `main` per LE branch model; (3) production `IP_HASH_SALT` set + `VITE_HOMEPAGE_ALLY_ENABLED=true` flip — both require explicit Oliver go-ahead. **Cross-cutting:** if the OpenRouter migration (sibling branch `feat/openrouter-migration`) lands first, this endpoint also needs to swap from `@anthropic-ai/sdk` to `lib/openrouter.ts`.
