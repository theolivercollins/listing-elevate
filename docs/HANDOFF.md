...
Last updated: 2026-05-27 (FFmpeg-on-Vercel 500 fix: ffmpeg-static/ffprobe-static swap + DirectorModal library polish + PromptLab batch UX overhaul)
...
**2026-05-27 (latest): FFmpeg-on-Vercel 500 fix + Director library polish + PromptLab batch UX overhaul.** Three things landed together as PR #95 (merge commit `7b22a6b`). **(A) FFmpeg fix**: the recurring `FUNCTION_INVOCATION_FAILED` 500 on Director Generate (`api/admin/prompt-lab/assemble.ts`, `assemble-listing.ts`, plus cron `poll-lab-renders.ts` / `poll-scenes.ts`) is finally squashed. Root cause: `@ffmpeg-installer` and `@ffprobe-installer` resolve their platform subpackage at runtime via `require.resolve("@ffXXX-installer/<platform>-<arch>/<binary>")` — Vercel's static file tracer can't follow that, even with `includeFiles` globbing the linux-x64 subpackage in. Swapped to `ffmpeg-static` + `ffprobe-static` which export a path computed from `__dirname` at import time (a real file reference the tracer can see), combined with `vercel.json` `includeFiles` pointing at the exact binary files `node_modules/{ffmpeg-static/ffmpeg,ffprobe-static/bin/linux/x64/ffprobe}`. `lib/utils/ffmpeg.ts` and `lib/utils/image-processing.ts` both updated; lazy bin helpers in ffmpeg.ts so unit tests and dev shells with system ffmpeg keep working. **(B) DirectorModal library polish**: capped library to ~6 visible cards (`maxHeight: 420` + `overflowY: auto`) so 20+ iteration batches don't stretch the modal off-screen (Oliver flagged on Hendry Creek). Swapped confusing "Session 8 · 2" labeling for friendly names: uses the session's own `label` field when set, else a short hash from `image_path` (e.g. `prompt-lab/<uid>/...-ab12cd.jpg` → `ab12cd`). Header now shows "Library — N clips · scroll for more" when N > 6. **(C) PromptLab batch UX overhaul** (already shipped on the branch, came along with the merge): sticky `<LabSubNav>` (top: 0), concise PageHeading + slim FileDropZone single-row, batch opens as a full-width drawer ABOVE the collapsed grid (not in-line under the batch tile), 🎬 Direct batch button mounted in the drawer. Build clean (vite 7.49s). Branch `feat/promptlab-batch-ux` merged to `main` per Oliver's go-ahead.

**Verify in prod after deploy lands (~6 min):** click Director Generate on any batch in `/dashboard/development/prompt-lab?v=v1.1` → should produce an assembled MP4 without a 500. Open DirectorModal on Hendry Creek (or any batch with 20+ iterations) → library panel should cap at ~6 visible cards with scroll hint in the header.
