...
Last updated: 2026-06-12 (market-update end-to-end verified from real-PDF inception — extract→validate→fill→faq→drafts→run row, two-step confirm publish/send wired in RunDetail, template token-coverage validation on upload; commit 6ed4f3e on `feat/market-update-workflow`)
...
Last updated: 2026-06-11 (market-update smoke+db-verify hardened — cost_events write verified, zero unresolved token assertion, multi-template .single() bug fixed; commit 6ed4f3e on `feat/market-update-workflow`)
...
**2026-06-12 (latest): Monthly Market Update workflow — fully verified end-to-end from real-PDF inception, awaiting Oliver's go-ahead to push + open PR.** Branch `feat/market-update-workflow` is 11 commits ahead of origin, containing the complete MU E2E pipeline (commit 6ed4f3e): (1) **Real-PDF E2E verified** — extract→validate→fill→faq→drafts→run row flow tested against real Stellar MLS reports + real Supabase + real Claude; zero unresolved token assertions; cost_events write verified. (2) **Last-mile publish/send wired in RunDetail** — from the per-run detail page, each draft now shows live status (draft/published/sent) + two-step confirmation buttons for Publish to Sierra and Send via Sendy, directly wired to the existing rails. (3) **Template token-coverage validation on upload** — in-page template upload validates uploaded templates (Blog_Template_MU.html / Email_Template_MU.html with canonical {{TOKEN}} names) for full token coverage; clear errors surface unknown or missing tokens. (4) **All verification gates passed** — smoke tests + full backend run + security audit + code review; no regressions. **Next action:** await Oliver's green light to `git push origin feat/market-update-workflow`, then open a PR from `feat/market-update-workflow` → `main` (11 commits, no migration needed). After merge, the workflow is live with full E2E capability — Oliver uploads 3 regional PDFs at `/dashboard/studio/blog/market-update` and completes the full monthly job (PDF → analyze → validate → generate → publish/send) from one page with confirmations.

**2026-06-11: Assembly quality drop — diagnosed and fixed on branch `fix/max-quality-assembly`.** Root-cause of the 5019 San Massimo Drive quality regression (final assembled video visibly softer than source clips). Measured via ffprobe: 6 of 7 source clips were sub-1080p 3:2 (1172x784, 0.92 MP) and got cover-upscaled 1.64x onto the 1920x1080 Creatomate canvas — the dominant loss. Root cause: Kling i2v copies the INPUT image's aspect ratio regardless of the `aspect_ratio` field; the existing `source-aspect.ts` 16:9 pre-crop (already used for Seedance via `forceSourceAspectRatio`) was never applied to any Kling path. Secondary finding: Creatomate's fixed ~6 Mbps output has no quality knob (Creatomate's API has no bitrate/quality param). Also: Atlas HTTP 402 (insufficient balance) on this run caused every v1.1 Seedance scene to fail over to native Kling, compounding the issue — **top up the Atlas balance as an ops action.**

**Fixes shipped on the branch (commits `d47f097`→`6657e3a`):**
- `lib/providers/kling.ts` — `generateClip` now routes `sourceImageUrl` through `ensureSourceAspectRatio` (16:9 center-crop) before submitting to Kling image2video, same pattern as Seedance.
- `lib/providers/atlas.ts` — ALL Atlas-routed Kling SKUs (kling-v3-pro, kling-v2-6-pro, kling-v2-1-pair, kling-v2-master, kling-v3-std, kling-o3-pro) gain `forceSourceAspectRatio: "16:9"`, including BOTH start and end frames on paired SKUs. Stale "geometry is fixed in-model" comments removed.
- `lib/providers/shotstack.ts` — `buildShotstackConcatTimeline`, `buildShotstackTimeline`, `buildShotstackJustListedTimeline` all force `quality: "high"` (was missing — Shotstack defaults to "medium") and `fps: 24` (matches measured source clips; Shotstack default is 25, which forces a resample pass).
- `src/lib/labModels.ts` — UI-side model metadata updated: removed stale per-SKU resolution/aspect claims; added Kling pixel-budget fact (0.92 MP for v2-master/native, 2.07 MP for v2.6-pro/v3-pro, both shaped by input aspect).
- New tests: `lib/providers/__tests__/kling-source-aspect.test.ts`, `lib/providers/__tests__/shotstack-quality.test.ts`, plus additions to existing atlas/creatomate suites.
- Diagnosis session note: `docs/sessions/2026-06-11-assembly-quality-drop-diagnosis.md`.

**PROD GATES:** (1) **Top up Atlas balance** (HTTP 402 is why 6/7 scenes fell to native Kling on this run). (2) Merge branch to main (no migration, no new env vars). (3) Live end-to-end probe — ffprobe the first Atlas Kling clip after the crop lands and confirm 1920x1080 (2.07 MP SKUs) or 1280x720 (v2-master/native, 0.92 MP fixed budget). Rollback: revert commits `938084a` + `2be6932` + `d47f097`.

**2026-06-04 (latest): Monthly Market Update workflow (blog engine) — SHIPPED to `main` + PROD (`www.listingelevate.com`, deploy `6839ade`, READY).** Promotion was a direct `feat → main` `--no-ff` merge (Oliver-approved live) because dev/staging are long-stale (269/268 behind main, 0 unique commits — pre-existing hygiene drift, left as-is). Prod smoke: homepage 200, `/api/blog/market-update/config` → app-level `{"error":"Unauthorized"}` (route + serverless live, not Vercel-protection), dynamic `runs/[id]` rewrite works, page route serves. **Still HITL by design:** the first real Sierra publish + Sendy send are actions Oliver triggers per-draft in-platform; and the two seeded MU templates are functional defaults — Oliver should upload the real `Blog_Template_MU.html`/`Email_Template_MU.html` in-platform (`/dashboard/studio/blog/templates` + `/email/templates`) keeping the canonical `{{TOKEN}}` names. Upload the 3 regional Stellar MLS stat-report PDFs (Charlotte County, The Isles=BSI+PGI, Deep Creek) → Claude tool-use extracts a schema-forced `RegionMetrics` per region → **deterministic math validation** re-derives every MoM/YoY and flags drift (error → `needs_review`, blocks drafts; warning → surfaced) → deterministic `{{TOKEN}}` fill of MU templates → AI rewrites only the FAQ block → images stripped for Isles/Deep Creek → creates 3 blog drafts + 1 CC email draft that ride the **existing** HITL → Sierra-publish / Sendy-send rails (reused unchanged). New: `lib/blog-engine/market-update/*` (types/format/fill/validate/strip-images/extract/faq/run), `api/blog/market-update/{config,runs,runs/[id],runs/[id]/generate}`, UI at `/dashboard/studio/blog/market-update` (+ "Market Update" link in the Blog header), migration **074** (mu_regions + market_update_runs + seeded 3 regions + default blog/email MU templates), cost stages `blog_mu_extract`/`blog_mu_faq`. **Migration 074 IS applied to the shared Supabase** (additive only; Oliver-approved). **Verified:** 19 unit/integration tests; 2 real-PDF smokes (`scripts/blog/mu-smoke.ts`) — full BSI+PGI report reconciles 0-issue, condensed CC graphic correctly → needs_review; full backend run against **real Supabase + real Claude** (`scripts/blog/mu-db-verify.ts`) created+verified+cleaned-up 3 blog + 1 email drafts against the real schema; cost lands in `cost_events`. Caught+fixed a real bug (float costCents vs integer `cost_events.cost_cents`). tsc + vite build clean; full suite 640 pass / 5 pre-existing fails. Spec: `docs/specs/2026-06-04-market-update-workflow-design.md`.
