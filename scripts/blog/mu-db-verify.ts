...
async function main() {
  ...
  const { data: blogTpl } = await supabase.from("blog_templates")
    .select("id").eq("site_id", siteId).eq("metadata->>kind", "market_update").eq("metadata->>mu_role", "blog").single();
  const { data: emailTpl } = await supabase.from("email_templates")
    .select("id").eq("site_id", siteId).eq("metadata->>kind", "market_update").eq("metadata->>mu_role", "email").single();
  if (!regionCfg?.length || !blogTpl || !emailTpl) throw new Error("missing seed config/templates");
  ...
  console.log("▶ analyzeRun (real Claude extraction ×3)…");
  const analyzed = await analyzeRun({
    supabase, siteId, periodMonth: 10, periodYear: 2025,
    blogTemplateId: blogTpl.id, emailTemplateId: emailTpl.id, regions,
  });
  console.log(`  run ${analyzed.runId} · status=${analyzed.status} · cost=${analyzed.costCents}¢`);
  ...
  console.log(`  created posts=${gen.postIds.length} emails=${gen.emailIds.length} cost=${gen.costCents}¢`);
