...
async function handlePost(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Create property row with pending_payment status + unpaid stripe status.
    // The pipeline is NOT triggered here — the Stripe webhook does that after
    // checkout.session.completed is received and verified.
    ...
    // 'requested' so the team queue at /dashboard/voice-clones surfaces them.
