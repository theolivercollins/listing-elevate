 ...
 * auth-gate.test.ts
 *
 * F4 security fix — asserts that each stats read endpoint rejects
...
beforeEach(() => {
  vi.clearAllMocks();
  // Default: unauthenticated — sends 401 and returns null.
...
describe('GET /api/stats/overview — auth gate (F4)', () => {
  it('returns 401 for unauthenticated request', async () => {
    const res = makeRes();
    await overviewHandler(makeReq(), res as VercelResponse);
    expect(res._last().status).toBe(401);
  });
});

describe('GET /api/stats/daily — auth gate (F4)', () => {
  it('returns 401 for unauthenticated request', async () => {
    const res = makeRes();
    await dailyHandler(makeReq(), res as VercelResponse);
    expect(res._last().status).toBe(401);
  });
});
