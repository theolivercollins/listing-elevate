 ...
 * skip.test.ts
 *
 * Tests for POST /api/scenes/[id]/skip — F4 auth gate.
 *
...
const { default: handler } = await import('../skip.js');
...
beforeEach(() => {
  ...
  mockFrom.mockImplementation(() => makeSceneChain({ property_id: 'prop-1', scene_number: 5 }));
});
...
describe('POST /api/scenes/[id]/skip — F4 auth gate', () => {
  ...
  it('proceeds and returns 200 on non-prod when LE_ALLOW_NONPROD_WRITES=true', async () => {
    ...
    expect(mockUpdateScene).toHaveBeenCalledWith('scene-xyz', { status: 'qc_pass', clip_url: null });
    const last = res._last();
    expect(last.status).toBe(200);
    expect((last.body as Record<string, unknown>).message).toBe('Scene skipped');
  });
});
